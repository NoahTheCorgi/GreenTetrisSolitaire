...
class TetrisLevelAI:
    ...
    def __init__(self, board, level):
        ...
        self.boardevaluation = (
            None  # this is a list of 7 possible blocks in hierarchy ordered lists
        )
    ...
    def sort_on_eval(self, eval):  # sorting 7 things.
        ...
        for (
            block
        ) in eval:  # a block here is an list of the form [evaluation, block index]
    ...
    def evaluateBoard(self):  # update the boardevaluation and returns it
        ...
        eval = [
            [self.evaluateBlock(0) + 10, 0],
            [self.evaluateBlock(5) + 9, 5],
            [self.evaluateBlock(6) + 8, 6],
            [self.evaluateBlock(3) + 7, 3],
            [self.evaluateBlock(4) + 6, 4],
            [self.evaluateBlock(2) + 5, 2],
            [self.evaluateBlock(1) + 4, 1],
        ]
